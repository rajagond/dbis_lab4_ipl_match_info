...
const getMatchBasicByMatch_Id = function(req, res, next) { //= (req, res, next) =>
    const id = parseInt(req.params.match_id)
    const query = {
        text: `SELECT match.match_id, season_year, winner_team, toss_team, toss_name, win_margin, win_type, inn1_total, inn1_wickets, inn1_over, inn2_total, inn2_wickets, inn2_over
    FROM (SELECT match_id, season_year, e.team_name AS winner_team, f.team_name AS toss_team, toss_name, win_margin, win_type FROM match INNER JOIN team AS e ON e.team_id = match_winner INNER JOIN team as f ON f.team_id = toss_winner WHERE match_id = $1) AS match
